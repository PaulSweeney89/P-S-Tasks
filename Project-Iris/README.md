...
- The separation between the clusters of datapoints for the different flower types between the dataset variables enabled Fischer to classify and identify the iris species based on their petal & sepal features. The linear correlations seen between some variables would also allow for the estimating of an unknown  variable value by using other known variable values. 

**Further Comments:**

The work of Fischer and the Iris dataset is used extensively as a example for machine learning and statistical classification techniques, although not forming part of this project it would be possible to take the work further to develop a statistical model for predicting the class or species of iris based solely on it's measured features, i.e petal, sepal lengths and widths. 
It is used as an example of supervised learning, where the training dataset conisting of known inputs (Iris plant features) are mapped with the correct known outputs (Iris species) to form an algorithm that finds correlaltions between the inputs and outputs which can then be used to classify or identify the Iris plant species for an unknown set of input variables. 

From undertaking this project, I have experimented with different methods for reviewing & exploring the Iris Datasets, from producing histograms by using for loops to automate the process, to plotting multiple scatterplots on a single figure using subplots. Delving into fully customizing & editing plots, specifying axis labels & tick values, displaying plot grids, to producing much more convient and attractive plots using the seaborn library. 
Overall, this project provided a good introduction to programming with python, working with various python libraries and data analytics as a subject. 
The scope of the project provided plenty room for experimentation, research, problem solving & analysis of the Iris dataset. Apon further study within the GMIT curriculum and with further research into the subject of data analytics, I would like to return to the Iris dataset for further exploration.   
